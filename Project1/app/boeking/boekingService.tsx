...
// Realtime listener voor boekingen van een specifieke server en datum
export const fetchBoekingen = (
  serverNaam: string,
  datum: Date,
...
// Maakt een nieuwe boeking aan na controle op overlappingen
export const maakBoeking = async (booking: Omit<Boeking, "id">) => {
  ...
  const conflict = bestaandeBoekingenSnap.docs.some((item) => {
    ...
    if (bestaande.status === "geannuleerd") return false;
    return heeftOverlap(
      booking.startTijd,
      booking.eindeTijd,
      bestaande.startTijd,
...
export const annuleerBoeking = (bookingId: string) => {
...
export const markeerAfgerond = (bookingId: string) => {
  return updateDoc(doc(db, "boekingen", bookingId), {
    status: "afgerond",
    updatedAt: serverTimestamp(),
  });
...
export const fetchMijnBoekingen = async (userId: string): Promise<Boeking[]> => {
  const q = query(
    boekingenRef,
    where("userId", "==", userId),

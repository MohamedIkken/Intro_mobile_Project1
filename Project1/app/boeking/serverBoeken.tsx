...
import { View , Text, ActivityIndicator, Modal, Platform, Pressable, SafeAreaView, ScrollView, StatusBar, TextInput, TouchableOpacity} from "react-native";
...
export default function ServerBoeken() {
    const { filters, slots, selectedSlot, loading, error,
            setFilters, searchAvailability, selectSlot, reserveSlot, clearError } = useBooking();
    const { user } = useAuth();
    ...
    const getServerBeschikbaarheid = (serverNaam: string): "Beschikbaar" | "Vol" => {
        ...
        const beschikbareSlots = slots.filter(slot => slot.beschikbaarheid === "beschikbaar");
        return beschikbareSlots.length > 0 ? "Beschikbaar" : "Vol";
    };

    const gekozenServer = servers.find((s: Server) => s.naam === filters.serverNaam);
            ...
            <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
                    ...
                    {serversLoading ? (
                        ...
                        <>
                            {gefilterdeServers.map((srv: Server) => {
                                ...
                                const disabled = filters.datum && !loading && filters.serverNaam === srv.naam && beschikbaarheid === "Vol";
                                ...
                                return (
                                    ...
                                    >
                                        <View style={[
                                            s.serverDot, 
                                            beschikbaarheid === "Beschikbaar" ? s.dotGroen : 
                                            beschikbaarheid === "Vol" ? s.dotOranje : s.dotGrijs
                                        ]} />
                                        <View style={s.serverInfo}>
                                            <Text style={[s.serverNaam, disabled && s.dimTekst]}>{srv.naam}</Text>
                                            <Text style={s.serverSpecs}>{srv.specs}</Text>
                                        </View>
                                        {geselecteerd && <Text style={s.checkmark}>✓</Text>}
                                        {!geselecteerd && !disabled && <Text style={s.pijlTekst}>›</Text>}
                                    </TouchableOpacity>
                                );
                            })}
                ...
                {gekozenServer && (
                    <View style={s.sectie}>
                        ...
                        <TouchableOpacity style={s.datumKnop} onPress={() => setToonDatumPicker(true)}>
                        ...
                        <View style={s.duurRij}>
                            {DUUR_OPTIES.map((min) => (
                                ...
                                >
                                    <Text style={[s.duurTekst, filters.duur === min && s.duurTekstActief]}>
                                        {min}m
                                    </Text>
                                </TouchableOpacity>
                            ))}
                ...
                {gekozenServer && filters.datum && (
                    <View style={s.sectie}>
                        ...
                        {!loading && slots.length === 0 && (
                            <Text style={s.leegTekst}>Geen slots beschikbaar voor deze dag.</Text>
                        )}
                            ...
                            <View style={s.slotsGrid}>
                                {slots.map((slot) => {
                                    ...
                                    return (
                                        <TouchableOpacity
                                            key={slot.id}
                                            style={[s.slotKnop, actief && s.slotKnopActief, bezet && s.slotKnopBezet]}
                                            onPress={() => !bezet && selectSlot(slot)}
                                            disabled={bezet}
                                            activeOpacity={bezet ? 1 : 0.75}
                                        >
                                            <Text style={[s.slotTijd, actief && s.slotTijdActief, bezet && s.dimTekst]}>
                                                {formatTijd(slot.startTijd)}
                                            </Text>
                                            <Text style={[s.slotStatus, bezet && s.slotStatusBezet]}>
                                                {bezet ? "bezet" : actief ? "gekozen" : "vrij"}
                                            </Text>
                                        </TouchableOpacity>
                                    );
                                })}
                ...
                {selectedSlot && (
                    <View style={s.sectie}>
                        ...
                        <TouchableOpacity style={s.boekKnop} onPress={() => setBevestigModal(true)}>
                ...
                <View style={s.modalOverlay}>
                    <View style={s.modalKaart}>
                        ...
                        <Text style={s.modalTekst}>
                            {gekozenServer?.naam} van {selectedSlot ? formatTijd(selectedSlot.startTijd) : ""} tot {selectedSlot ? formatTijd(selectedSlot.eindeTijd) : ""}
                        </Text>
                        <View style={s.modalKnoppen}>
                            <Pressable style={s.modalAnnuleer} onPress={() => setBevestigModal(false)}>
            ...
            <Modal transparent visible={geboektModal} animationType="fade">
                <View style={s.modalOverlay}>
                    <View style={s.modalKaart}>
                        ...
                        <Pressable style={s.modalBevestig} onPress={() => setGeboektModal(false)}>
...
const s = StyleSheet.create({
    ...
    headerLabel:     { fontSize: 12, color: "#8888AA", fontWeight: "700", letterSpacing: 1, textTransform: "uppercase", marginBottom: 6 },
    headerTitle:     { fontSize: 28, fontWeight: "bold", color: "#FFFFFF" },

    sectie:          { marginTop: 20 },
    sectieLabel:     { fontSize: 12, color: "#8888AA", fontWeight: "700", letterSpacing: 1, textTransform: "uppercase", marginBottom: 10 },

    zoekRij:         { flexDirection: "row", alignItems: "center", backgroundColor: "#131320", borderRadius: 8, borderWidth: 1, borderColor: "#1E1E30", paddingHorizontal: 12, height: 48, marginBottom: 10 },
    ...
    serverKaart:         { flexDirection: "row", alignItems: "center", backgroundColor: "#131320", borderRadius: 8, borderWidth: 1, borderColor: "#1E1E30", padding: 15, marginBottom: 8 },
    serverKaartActief:   { borderColor: "#2E6BFF", backgroundColor: "rgba(46, 107, 255, 0.1)" },
    ...
    serverInfo:          { flex: 1 },
    serverNaam:          { fontSize: 14, fontWeight: "600", color: "#FFFFFF", marginBottom: 2 },
    ...
    leegTekst:           { color: "#8888AA", fontSize: 13, textAlign: "center", paddingVertical: 20 },

    datumKnop:       { flexDirection: "row", alignItems: "center", backgroundColor: "#131320", borderRadius: 8, borderWidth: 1, borderColor: "#1E1E30", padding: 15, marginBottom: 12 },
    datumIcon:       { fontSize: 16, marginRight: 10 },
    datumTekst:      { flex: 1, fontSize: 16, color: "#FFFFFF" },

    duurRij:         { flexDirection: "row", gap: 10 },
    duurKnop:        { flex: 1, paddingVertical: 14, borderRadius: 8, borderWidth: 1, borderColor: "#1E1E30", alignItems: "center", backgroundColor: "#131320" },
    duurKnopActief:  { borderColor: "#2E6BFF", backgroundColor: "rgba(46, 107, 255, 0.1)" },
    duurTekst:       { fontSize: 14, color: "#8888AA", fontWeight: "600" },
    duurTekstActief: { color: "#FFFFFF", fontWeight: "bold" },

    slotsGrid:       { flexDirection: "row", flexWrap: "wrap", gap: 10 },
    slotKnop:        { width: "23%", aspectRatio: 1.2, borderRadius: 8, borderWidth: 1, borderColor: "#1E1E30", backgroundColor: "#131320", alignItems: "center", justifyContent: "center" },
    slotKnopActief:  { borderColor: "#2E6BFF", backgroundColor: "rgba(46, 107, 255, 0.1)" },
    slotKnopBezet:   { backgroundColor: "#0F0F1A", borderColor: "#1E1E30", opacity: 0.4 },
    slotTijd:        { fontSize: 13, fontWeight: "bold", color: "#FFFFFF" },
    slotTijdActief:  { color: "#FFFFFF" },
    slotStatus:      { fontSize: 9, color: "#2E6BFF", marginTop: 3, textTransform: "uppercase", letterSpacing: 0.5 },
    slotStatusBezet: { color: "#8888AA" },

    samenvattingKaart:  { backgroundColor: "#131320", borderRadius: 8, borderWidth: 1, borderColor: "#1E1E30", padding: 16, marginBottom: 12 },
    samenvattingRij:    { flexDirection: "row", justifyContent: "space-between", paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: "#1E1E30" },
    samenvattingLabel:  { fontSize: 12, color: "#8888AA" },
    samenvattingWaarde: { fontSize: 13, color: "#FFFFFF", fontWeight: "600" },

    doelInput:       { backgroundColor: "#131320", borderRadius: 8, borderWidth: 1, borderColor: "#1E1E30", padding: 15, color: "#FFFFFF", fontSize: 16, minHeight: 60, marginBottom: 14, textAlignVertical: "top" },

    boekKnop:        { backgroundColor: "#2E6BFF", borderRadius: 8, padding: 16, alignItems: "center" },
    boekKnopTekst:   { fontSize: 16, fontWeight: "bold", color: "#FFFFFF" },

    errorBalk:       { marginTop: 16, backgroundColor: "#1A0F0F", borderRadius: 8, borderWidth: 1, borderColor: "#FF4C4C", padding: 14 },
    ...
    modalKaart:         { backgroundColor: "#131320", borderRadius: 8, borderWidth: 1, borderColor: "#1E1E30", padding: 24, width: "85%", alignItems: "center" },
    modalTitel:         { fontSize: 18, fontWeight: "bold", color: "#FFFFFF", marginBottom: 8 },
    modalTekst:         { fontSize: 13, color: "#8888AA", textAlign: "center", marginBottom: 24 },
    modalKnoppen:       { flexDirection: "row", gap: 10, width: "100%" },
    modalAnnuleer:      { flex: 1, paddingVertical: 13, borderRadius: 8, borderWidth: 1, borderColor: "#1E1E30", alignItems: "center" },
    modalAnnuleerTekst: { color: "#8888AA", fontWeight: "600" },
    modalBevestig:      { flex: 1, paddingVertical: 13, borderRadius: 8, backgroundColor: "#2E6BFF", alignItems: "center" },
